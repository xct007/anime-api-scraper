 ...
 */
import { Axios, Config, API_SEARCH, API_ONGOING, API_POPULAR, API_POSTS, API_POST_DETAIL, API_VIDEO_ID, } from "./utils.js";
/**
 ...
 */
export const search = async (query, lang) => {
    let result;
    if (!lang) {
        lang = "id";
    }
 ...
 * @returns {Promise<Object>}
 */
export const ongoing = async (lang) => {
    let result;
    if (!lang) {
        lang = "id";
    }
 ...
 * @returns {Promise<Object>}
 */
export const popular = async (lang) => {
    let result;
    if (!lang) {
        lang = "id";
    }
...
export const detail = async (id) => {
    let result;
    let data = await Axios.get(API_POSTS(id), {
        ...Config,
    }).catch((e) => {
        return e.response;
    });
