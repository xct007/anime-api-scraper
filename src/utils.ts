...
const API_URL =
	"https://pencarinafkah.xyz/com.animeku.animechannelsubindoandsubenglish";

const API_LATEST = async (
	page: number,
	limit: number
...
const API_POST = async (vid: number): Promise<string | boolean> => {
	const Data = await keyApi();
	if (Data) {
...
const API_VIDEOS = async (id: number): Promise<string | boolean> => {
	const Data = await keyApi();
	if (Data) {
...
async function keyApi(): Promise<RespOnKey> {
	const { data } = await axios.get(API_URL, {
		headers: {
			"User-Agent":
				"Dalvik/2.1.0 (Linux; U; Android 12; M2007J20CG Build/SKQ1.211019)",
		},
	});
	if (data) {
...
const FetchData = async (url: string): Promise<any> => {
	const { data } = await axios.get(`${url}`, {
		headers: {
			...
			"User-Agent":
				"Dalvik/7.1.12.1.0 (com.animeku.animechannelsubindoandsubenglish U; Android ; 20175 Build/NMF260)",
